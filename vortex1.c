...
// todo: формулы почему-то где-то даёт бесконечность в энергиях в расчёте при маленькой энергии внешнего магнитного поля
...
//		s = ((a[n - 1].zn == 'o') ? 1 : -1) * e3(r,a[n - 1].l) + ((a[i].zn == 'o') ? 1 : -1) * e3(r,a[i].l) - f;
		...
		v = t;
/*
#if XG & 2	// 1	-- отражают границы по X
		...
		v = t;
#endif
#if XG & 1	// 2	-- отражают границы по Y
		r = rrb(a[n - 1].x,a[n - 1].y,a[i].x,-a[i].y);
		if (a[i].zn == a[n - 1].zn)
			s = e3(r,a[n - 1].l) + e3(r,a[i].l) - f;
		else
...
double tote3(struct vortex *a, int n) {
	...
	for (j = 0; j < n; j++)
	{
		for (i = 0; i < j; i++)
		{
			...
			v = t;
/*
#if XM & 2	// 1	-- отражают границы по X
			...
			v = t;
#endif
#if XM & 1	// 2	-- отражают границы по Y
			r = rrb(a[j].x,a[j].y,a[i].x,-a[i].y);
			if (a[j].zn == a[i].zn)
				s = e3(r,a[j].l) + e3(r,a[i].l) - f;
			else
