...
// размеры
#define X	5e-3		// ширина	: см
#define Y	5e-3		// высота	: см
...
#define U02	0.01		// U0	: эВ		// нижний предел в распределении потенциалов дефектов
...
#define E1	m1 * h		// E1(H,λ,x: эВ	энергия от магнитного поля если все границы переодичны
//#define E1X	m1 * h * (1 + cosh((x - (X / 2)) / l) / cosh(X / (2 * l)))		// E1(H,λ,x)	: эВ	энергия от магнитного поля если границы не периодичны по X
//#define E1Y	m1 * h * (1 + cosh((y - (Y / 2)) / l) / cosh(Y / (2 * l)))		// E1(H,λ,x)	: эВ	энергия от магнитного поля если границы не периодичны по X
#define E1X	- m1 * h * sinh((x - (X / 2)) / l) / sinh(X / (2 * l))		// E1(H,λ,x)	: эВ	энергия от магнитного поля если границы не периодичны по X
#define E1Y	- m1 * h * sinh((y - (Y / 2)) / l) / sinh(Y / (2 * l))		// E1(H,λ,y)	: эВ	энергия от магнитного поля если границы не периодичны по Y
...
// примеси
#define NP	2500		// количество примесей	: штук
...
// границы
#define XG	0		// определяет характер границ
...
#define XM	0		// определяет характер отражений // fail: не работает так как надо
// 0	-- нет отражающих границ
// 1	-- отражают границы по Y
// 2	-- отражают границы по X
// 3	-- все границы отражают
...
/* энергия взаимодействия с магнитным полем E1(H) */
...
/* энергия взаимодействия вихря с примесью E4(r,ξ) */
...
double eone3(struct vortex *a, int n) {
	...
	for (int i = 0; i < n - 1; i++)
	{
		...
		if (a[i].zn == a[n - 1].zn)
			...
			s = -e3(r,a[n - 1].l) - e3(r,a[i].l) - f;
//		s = ((a[n - 1].zn == 'o') ? 1 : -1) * e3(r,a[n - 1].l) + ((a[i].zn == 'o') ? 1 : -1) * e3(r,a[i].l) - f;
		t = v + s;
		f = (t - v) - s;
		v = t;

#if XM & 2	// 2	-- отражают границы по Y
		...
		v = t;
#endif
#if XM & 1	// 1	-- отражают границы по X
		r = rrb(a[n - 1].x,a[n - 1].y,-a[i].x,a[i].y);
		if (a[i].zn == a[n - 1].zn)
			s = e3(r,a[n - 1].l) + e3(r,a[i].l) - f;
		else
	...
	for (j = 0; j < n; j++)
		for (i = 0; i < j; i++)
		{
			...
			if (a[i].zn == a[j].zn)
				...
				s = -e3(r,a[j].l) - e3(r,a[i].l) - f;
//			s = ((a[j].zn == 'o') ? 1 : -1) * e3(r,a[j].l) + ((a[i].zn == 'o') ? 1 : -1) * e3(r,a[i].l) - f;
			t = v + s;
			f = (t - v) - s;
			v = t;
...
//#if XG & 2
//	x = (double)rand() / RAND_MAX * L0 + (rand() % 2 ? 0 : X - L0);
//#else
	x = (double)rand() / RAND_MAX * X;
//#endif
//#if XG & 1
//	y = (double)rand() / RAND_MAX * L0 + (rand() % 2 ? 0 : Y - L0);
//#else
...
//				v[nm] = new(v[nm],&nn[nm]);
			...
			case 2: // анигиляция
				...
				if (xx && yy)
				{
					...
					if (rrr(v[nm][i].x,v[nm][j].y,v[nm][j].x,v[nm][j].y) <= DS)
					{
